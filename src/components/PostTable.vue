          ...
          <template v-for="(post, index) in postList">
            <tr :key="post.id">
              ...
              <td>
                ...
                <button class="btn-default btn-offline" @click="offlinePostData(post.id, index)"
                        v-if="post.status === 'PUBLISHED'">下线</button>
                <button class="btn-default btn-publish" @click="publishPostData(post.id, index)" v-else>发布</button>
                <button class="btn-default btn-delete" @click="deletePostData(post.id, index)">删除</button>
              </td>
            </tr>
          </template>
        </tbody>
...
export default {
  ...
  methods: {
    ...
    offlinePostData: function (postId, index) {
      this.$msgBox.showMsgBox({
        title: '确认下线',
        content: '确认是否下线该篇文章？'
      }).then(async () => {
        let res = await this.offlinePost(postId);
        if (res.success === 1) {
    ...
    publishPostData: async function (postId, index) {
      this.$msgBox.showMsgBox({
        title: '确认发布',
        content: '确认是否发布该篇文章？'
      }).then(async () => {
        let res = await this.publishPost(postId);
        if (res.success === 1) {
    ...
    deletePostData: async function (postId, index) {
      this.$msgBox.showMsgBox({
        title: '确认删除',
        content: '删除后无法恢复，确认删除该篇文章？'
      }).then(async () => {
        let res = await this.deletePost(postId);
        if (res.success === 1) {
